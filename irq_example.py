...
)  # Tells our program that this is a PIO routine and to set the pin low on activation.
def pin_activator():
    ...
    jmp(
        x_dec, "Jump_Point"
    )  # if x is NOT 0(zero), remove one (-1) from X and jump back to "Jump_Point", Else, proceed.
...
#                        --- PIO Block 1 ---
sm_4 = rp2.StateMachine(5, pin_activator, freq=20000, set_base=machine.Pin(1))
# Calls rp2 and Instantiate a statemachine
...
sm_5 = rp2.StateMachine(6, delay, freq=20000)
